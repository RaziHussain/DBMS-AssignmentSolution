...
Select psg.Passenger_name, psg.Boarding_city, psg.Destination_City, psg.Bus_Type, pt.price from passenger as psg
join price_table as pt on psg.Distance = pt.Distance;

/*What are the passenger name/s and his/her ticket price who travelled in the Sitting bus 
for a distance of 1000 KM s*/

Select psg.Passenger_name, pt.Price from passenger as psg 
join Price_table as pt on
...
psg.Boardng_city, pt.Bus_type, pt.Price FROM passenger as psg inner join price_table as pt USING (distance)
...
SELECT passenger.distance, price_table.price,
CASE
...
FROM passenger join price_table on passenger.distance = price_table.distance
